async function fetchMenuData() {
    try {
        ...
        console.error("Errow while fetchin food data:", error);
    }
}
...
function renderMenuItems(items, categoryName) {
    ...
    <div class="menu-item-footer">
        <h3 class="menu-item-price"><span>$</span>${item.price.toFixed(2)}</h3>
...
function renderInvoice() {
    ...
    cart.forEach((item, index) => {
        ...
        subTotal += itemTotalPrice; //subTotal = subTotal + itemTotalPrice;
        ...
        <img src="${item.image}" width="200px" alt="${item.name}" class="invoice-item-img" />
        <div class="invoice-item-details">
            <h3>${item.name}</h3>
            <div class="invoice-item-quantity">
                <button class="quantity-btn decrease" data-index="${index}">-</button>
                <input type="text" class="qunatity-input" value="${item.quantity}" />
